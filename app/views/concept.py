...
def concept(request, conceptid):
    ...
    if request.method == 'POST':
        ...
        if len(request.FILES) > 0:
            ...
            if imagefile:
                language = models.DLanguages.objects.filter(languageid__iexact = request.LANGUAGE_CODE)
                value = models.FileValues(valueid = str(uuid.uuid4()), value = request.FILES.get('file', None), conceptid_id = conceptid, valuetype_id = 'image',languageid_id = language[0])
                value.save()
